  ...
  login(credentials: SignInPayload): Observable<AuthResponse> {
    ...
    return this.http.post<AuthResponse>(`${this.apiUrl}/${endpoint}`, credentials).pipe(
      ...
      catchError(error => {
        ...
        // Affiche une notification toast visible pour le compte inactif, même si le composant login est détruit
        if (
          ...
          setTimeout(() => {
            this.notificationService.success("Votre compte est inactif. Veuillez contacter l'administrateur.", 10000);
            console.log('Toast vert affiché (depuis AuthService) : compte inactif');
          }, 100);
        }
