          ...
          <tbody>
            <tr *ngFor="let member of filteredMembers">
              ...
              <td class="actions-cell">
                <!--
                  Bouton unique pour activer/désactiver (soft delete/restauration) le membre.
                  - Si membre actif : icône bonhomme vert (user-check), clic = désactive (isActive=false)
                  - Si membre inactif : icône bonhomme barré orange (user-slash), clic = restaure (isActive=true)
                  Le changement est instantané partout grâce au data-binding.
                -->
                <button 
...
export class AdminMembersComponent implements OnInit {
  ...
  constructor(
    ...
    // On écoute les événements de navigation Angular.
    // Après chaque navigation, si on est sur /admin/members, on recharge la liste.
    // Le cast 'as NavigationEnd' est nécessaire pour que TypeScript reconnaisse les propriétés spécifiques.
  ...
  loadMembers(): void {
    ...
    this.memberService.getAllMembers().subscribe({
      next: (response: any) => {
        // Correction : la réponse peut être un tableau ou un objet avec une propriété data
        let users: any[] = [];
  ...
  /**
   * Active ou désactive un membre (soft delete/restauration)
   * - Si actif : passe inactif (isActive=false) via updateMember
   * - Si inactif : restauration via restoreMember (PATCH)
   * Le changement est visible partout grâce au data-binding et à la notification du dashboard.
   */
  toggleMemberStatus(member: Member) {
    console.log('toggleMemberStatus appelé pour', member);
    if (member.isActive) {
      // Désactivation (soft delete) : on passe isActive à false
      this.memberService.updateMember(member.id, { isActive: false }).subscribe({
        next: () => {
      ...
      // Restauration : on appelle l'endpoint PATCH /security/restore/:id
  ...
  /**
   * Restaure un membre inactif (soft deleted) en le réactivant côté backend
  ...
  restoreMember(member: Member) {
    ...
    this.memberService.restoreMember(member.id).subscribe({
      next: () => {
        ...
        this.loadMembers(); // Recharge la liste
      },
      error: (error: Error) => {
  ...
  /**
   * Formate l'affichage du nom de l'entreprise
   */
